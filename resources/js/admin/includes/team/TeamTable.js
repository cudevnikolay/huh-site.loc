...
import {switchTeamStatus} from "./SwitchTeamStatus";

let teamLengthCookieName = 'team_length';
let teamDefaultLength = 10;
const table = $('#team');

const teamTable = table.DataTable({
    "order": [[0, 'asc']],
    autoWidth: false,
    "oTeam": {
    ...
    "drawCallback": function (settings) {
        $('.team-status').on('click', function () {
            switchTeamStatus($(this).data('id'));
        });
    ...
    "columns": [
            ...
            render: function (data, type, row) {
                if (data.enabled === 'default') {
                    ...
                    return `<label class="switch">
                              <input class="team-status" data-id="${data.id}" type="checkbox" ${checked}>
        ...
        },
        {
            ...
            render: function (data, type, row) {
                ...
                if (data.delete) {
                    deleteButton = `<div>
                            <a href="${data.delete}" class="btn-sm btn-danger delete-team-button " data-toggle="modal" 
                            data-target="#myModal">
    ...
    "pageLength": getPageLength(teamLengthCookieName, teamDefaultLength),
    lengthMenu: [[10, 20, 50, 100, 500], [10, 20, 50, 100, 500]]
});

saveTableSizeToCookies(table, teamLengthCookieName);
